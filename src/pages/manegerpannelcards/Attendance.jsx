...
export default function Attendance() {
  const getEmployeeId = (emp) =>
    emp?.id || emp?.employeeId || emp?.uid || emp?.firebaseUid || "";

  const getEmployeeUid = (emp) =>
    emp?.firebaseUid || emp?.uid || getEmployeeId(emp) || "";
  ...
  const verifyFingerprintForPresent = async (emp) => {
    const uid = getEmployeeUid(emp);
    if (!uid) throw new Error("Employee UID missing for fingerprint verification");

    const optionsRes = await fetch(`${API_BASE_URL}/biometric/auth-options/${uid}`);
    if (!optionsRes.ok) {
      const error = await optionsRes.json().catch(() => ({}));
      throw new Error(error.error || "Employee fingerprint is not registered");
    }

    const options = await optionsRes.json();
    const authResp = await startAuthentication({ optionsJSON: options });

    const verifyRes = await fetch(`${API_BASE_URL}/biometric/verify-auth/${uid}`, {
      method: "POST",
    ...
    if (!verifyRes.ok || verify.success !== true) {
      throw new Error(verify.error || "Fingerprint verification failed");
    }
  ...
  const setStatus = async (emp, status) => {
    ...
    try {
      ...
      let biometricUid = null;

      if (status === "Present") {
        biometricUid = await verifyFingerprintForPresent(emp);
        biometricVerified = true;
      }

      await managerApi.verifyAttendance({
        employeeId,
        uid: getEmployeeUid(emp),
        status,
        date: selectedDate,
        biometricVerified,
        biometricUid,
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">{emp.department || emp.dept || emp.role || "-"}</p>
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={savingId === getEmployeeId(emp)}
              >
