...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    // This is a single-page landing experience (/#about, /#services, etc.).
    ...
    const obs = new IntersectionObserver(
      (entries) => {
        ...
        if (best && best !== activeIdRef.current) setActiveId(best);
      },
      {
        // Bias towards the section around the top/middle of the viewport.
        root: null,
            ...
            <li key={link.name}>
              <Link
                ...
                className={`transition-colors duration-200 hover:text-yellow-400 ${
                  activeId === link.id ? "text-yellow-400" : ""
                }`}
              >
                {link.name}
              </Link>
